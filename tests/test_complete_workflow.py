...
async def run_complete_workflow(job_url: str, user_profile_path: str = "data/user_profile_resume_format.yaml"):
    """
    ...
    try:
        ...
        print(f"  Key requirements: {len(job_result.job_requirements.key_requirements)} items")
        print(f"  Technical skills: {len(job_result.job_requirements.technical_skills)} items")
    except Exception as e:
    ...
    try:
        ...
        print(f"✓ Successfully tailored resume")
    ...
    try:
        ...
        report_path = output_dir / f"{company_clean}_{role_clean}_{timestamp}_report.txt"
