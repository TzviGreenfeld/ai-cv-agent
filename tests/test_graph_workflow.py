...
async def test_workflow_success():
    ...
    mock_resume_data.candidate = {"name": "Test User", "title": "Senior Python Developer"}
    ...
    # Apply mocks
    with patch("ai_cv_agent.graph.workflow_graph.read_user_profile") as mock_read_profile, \
         patch("ai_cv_agent.graph.workflow_graph.convert_raw_resume_to_resume_data") as mock_convert, \
         patch("ai_cv_agent.graph.workflow_graph.JobParserAgent") as mock_parser_class, \
         patch("ai_cv_agent.graph.workflow_graph.ResumeTailoringAgent") as mock_tailoring_class, \
         patch("ai_cv_agent.graph.workflow_graph.generate_cv_html") as mock_generate_html, \
...
async def test_workflow_job_parse_failure():
    ...
    with patch("ai_cv_agent.graph.workflow_graph.read_user_profile"), \
...
def test_graph_structure():
    ...
    expected_nodes = {
        "load_profile", "parse_job", "tailor_resume", 
        "generate_html", "export_pdf", "error_sink", "success_sink"
    }
