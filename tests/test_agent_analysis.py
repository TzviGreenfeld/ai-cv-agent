...
def test_standalone_job_analysis():
    ...
    try:
        result = agent.run(f"Analyze this job posting and tell me the key requirements: {job_url}")
        ...
        try:
            ...
            if result.strip().startswith('{'):
                ...
                print(f"  Key Requirements: {', '.join(job_data.get('key_requirements', []))}")
                print(f"  Technical Skills: {', '.join(job_data.get('technical_skills', []))}")
                print(f"  ATS Keywords: {', '.join(job_data.get('keywords_for_ats', []))}")
        except:
...
def main():
    ...
    print("\nRunning Test 2...")
    tests_passed.append(("Complete Workflow with Explanations", test_complete_workflow_with_explanations()))
