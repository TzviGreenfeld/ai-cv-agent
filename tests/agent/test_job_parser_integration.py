...
class TestJobParserIntegration:
    ...
    async def test_parse_google_swe3_job(self, mock_job_content):
        ...
        with patch('ai_cv_agent.agent.job_parser_agent.read_job_description') as mock_fetch:
            ...
            assert "Software Engineer" in job.role or "SWE" in job.role, f"Role extracted: {job.role}"
            assert "III" in job.role or "3" in job.role or "Senior" in job.role, f"Level not identified in role: {job.role}"
            ...
            assert any("Bengaluru" in str(item) or "Hyderabad" in str(item) 
                      for item in [job.raw_description, job.nice_to_have, job.key_requirements])
            
            # Validate key requirements were extracted
            assert len(job.key_requirements) > 0, "No key requirements extracted"
            assert any("Bachelor" in req or "degree" in req for req in job.key_requirements), \
                f"Degree requirement not found in: {job.key_requirements}"
            assert any("2 years" in req or "experience" in req for req in job.key_requirements), \
            ...
            expected_skills_keywords = [
                "software development", "programming", "data structures", 
                "algorithms", "distributed computing", "system design",
                "cloud", "google cloud"
            ]
            found_skills_text = ' '.join(skill.lower() for skill in job.technical_skills)
            assert any(keyword in found_skills_text for keyword in expected_skills_keywords), \
            ...
            responsibility_keywords = ["code", "design", "review", "develop", "test", "debug"]
            found_responsibilities_text = ' '.join(resp.lower() for resp in job.main_responsibilities)
            assert any(keyword in found_responsibilities_text for keyword in responsibility_keywords), \
            ...
            assert any("cloud" in kw or "google cloud" in kw for kw in ats_keywords_lower), \
                f"Cloud keywords not found in ATS keywords: {job.keywords_for_ats}"
            
            # Check nice-to-have extraction
            assert any("Master" in nth or "PhD" in nth or "accessible" in nth 
            ...
            print(f"  Technical Skills ({len(job.technical_skills)}): {', '.join(job.technical_skills[:5])}...")
            print(f"  ATS Keywords ({len(job.keywords_for_ats)}): {', '.join(job.keywords_for_ats[:5])}...")
