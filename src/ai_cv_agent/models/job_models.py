...
class JobRequirements(BaseModel):
    ...
    keywords_for_ats: List[str] = Field(
        default_factory=list, description="Keywords for ATS optimization"
    )
    ...
    source_url: Optional[str] = Field(
        None, description="URL where job was fetched from"
    )
    parsed_at: datetime = Field(
        default_factory=datetime.now, description="When the job was parsed"
    )
    ...
    def get_all_keywords(self) -> List[str]:
        ...
        return list(
            set(self.keywords_for_ats + self.technical_skills + self.soft_skills)
        )
