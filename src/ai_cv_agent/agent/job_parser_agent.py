...
class JobParserAgent:
    ...
    async def parse_from_url(self, url: str) -> JobParseResult:
        ...
        try:
            ...
            if not job_text:
                return JobParseResult.error_result("Failed to fetch job description from URL")
    ...
    async def parse_from_text(
        ...
        try:
            # Prepare the prompt
            analysis_prompt = JOB_PARSER_PROMPT.format(
    ...
    def parse_from_text_sync(
        ...
        try:
            return loop.run_until_complete(
                self.parse_from_text(job_text, source_url)
            )
        finally:
            loop.close()
