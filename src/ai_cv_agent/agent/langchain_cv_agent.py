...
# Import prompts
from .prompts import SYSTEM_PROMPT, TAILORING_PROMPT, JOB_ANALYSIS_PROMPT
...
class LangChainCVAgent:
    ...
    def _create_tools(self):
        ...
        @tool
        def create_tailored_resume_complete(
            job_url: str,
            profile_path: str = "data/user_profile_resume_format.yaml",
            output_name: str = "test_resume",
        ) -> str:
            """
            ...
            try:
                ...
                import asyncio
                from ..tools.job_reader import (
                    read_job_description as _read_job_description,
                )
                ...
                if "```yaml" in tailored_yaml:
                    tailored_yaml = (
                        tailored_yaml.split("```yaml")[1].split("```")[0].strip()
                    )
                elif "```" in tailored_yaml:
                    tailored_yaml = (
                        tailored_yaml.split("```")[1].split("```")[0].strip()
                    )
                ...
                from ..tools.html_cv_builder import (
        ...
        def analyze_job_with_llm(job_description: str) -> tuple[str, dict]:
            ...
            try:
                ...
                try:
                    ...
                    return "Job analysis completed (non-structured)", {
        ...
        def create_detailed_tailoring_analysis(
            ...
            try:
                ...
                if not tailored_profile:
                    return (
                        "Error: Missing tailored profile data",
                        "Cannot generate report without tailored resume data",
                    )

                if not original_profile:
                    return (
                        "Error: Missing original profile data",
                        "Cannot generate report without original profile data",
                    )
